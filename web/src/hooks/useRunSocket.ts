...
function appendWarningLogs(nodeResults: RunStatusOut['node_results']) {
...
/**
 * Streams one run's progress into runStore (logs/status/per-node progress) and
 * graphStore (per-node status dots). Always resyncs via GET before subscribing to the
 * WS, since a run can finish before the socket connects — a non-"running" resync is
 * itself the terminal signal and the socket closes right after, with no further events
 * to wait for.
 *
...
export function useRunSocket(runId: string | null) {
  useEffect(() => {
    ...
    async function finalizeRun() {
      ...
      if (!cancelled && final) {
        useRunStore.getState().setLastNodeResults(final.node_results)
        appendWarningLogs(final.node_results)
      }
    }
    ...
    function applyEvent(event: RunEvent) {
      ...
      const { setNodeStatus } = useGraphStore.getState()
    ...
    async function resyncThenSubscribe() {
      ...
      if (current.status !== 'running') {
        ...
        useRunStore.getState().setStatus(current.status as RunStatus, current.error)
        useRunStore.getState().setLastNodeResults(current.node_results)
        appendWarningLogs(current.node_results ?? {})
        return
      }
      ...
      ws.onmessage = (msg) => {
        ...
        if (event.type === 'resync') {
          if (event.status && event.status !== 'running') {
            useRunStore.getState().setStatus(event.status as RunStatus, null)
            void finalizeRun()
          }
          return
        }
