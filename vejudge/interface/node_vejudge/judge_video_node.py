"""Video Judge Node executor — runs the video-modality metrics (M2, M4, M5, M6) over a
dataset.

Split from the old single Judge Node along the modality boundary that already existed
internally. See ``judge_text_node.py`` for its text counterpart and
``_concurrent_judging.py`` for the shared execution engine both wrap. Engine config
(kind/model/temperature/concurrency) comes from a required upstream LM Engine Node
(``lm_engine_node.py``) rather than this node's own params — see that module's docstring
for why its output is a plain config dict, not a live engine.
"""
...
from ...core.judge.registry import VIDEO_JUDGES
...
class VideoJudgeNodeExecutor(NodeExecutor):
    ...
    def run(self, ctx: NodeRunContext) -> NodeRunResult:
        ...
        if engine_config is None:
            return NodeRunResult(
                status="error",
                error="Video Judge Node requires an 'engine_config' input (wire an LM "
                "Engine Node's `engine_config` output)",
            )

        metrics = p.get("metrics") or list(VIDEO_JUDGES)
        unknown = [m for m in metrics if m not in VIDEO_JUDGES]
        if unknown:
            return NodeRunResult(
                status="error", error=f"Unknown or non-video metric id(s): {unknown}"
            )
        ...
        try:
            require_live(
                ctx.allow_live, context=f"Video Judge Node over {len(dataset)} item(s)"
            )
        ...
        engine = get_engine(
            engine_config.get("engine_kind", "gemini"), history=ctx.run.history,
            model=engine_config.get("model"), creds=load_creds(),
            max_tokens=int(engine_config.get("max_tokens") or 4096), **temp_kw,
        )

        def _should_skip(metric_id: str, sample: dict[str, Any]) -> bool:
            # Auto-skip per item when it has no rendered video.
...
def _dry_run_result(dataset: dict[str, Any], metrics: list[str]) -> NodeRunResult:
    ...
    return NodeRunResult(
        ...
        meta={
            "dry_run": True,
            "n_items": n_items,
            "estimated_calls": {"video_judge_calls": len(metrics) * n_items},
        },
    )
